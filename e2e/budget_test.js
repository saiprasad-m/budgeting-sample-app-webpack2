...
const budgetPage = require('./pages/Budget.js');
...
Data(categories).Scenario('Add catergories to Budgeting', async (I, current) => {
  // current!
...
Scenario('Calculate the Working Balance via script @all', async (I, budgetPage) => {
  const working = await budgetPage.calculateBalance();
});
